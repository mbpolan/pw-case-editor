...
bool Sprite::createFromGIFs(const QString &path) {
/*
	...
	Glib::Dir dir(path);
	std::vector<QString> files(dir.begin(), dir.end());
	...
	for (int i=0; i<files.size(); i++) {
		...
		// complete id string
		QString id=name+"_"+part;
		
		// form complete path to gif
		QString filePath=rpath+files[i];
...
void Sprite::addAnimationFromGIF(const QString &id, const QString &path) {
/*
...
void Sprite::addFrame(const QString &id, int time, const QPixmap &pixbuf) {
	Frame fr;
	fr.time=time;
	fr.pixbuf=pixbuf;
...
void Sprite::removeFrame(const QString &id, int index) {
